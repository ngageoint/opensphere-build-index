...
/**
 * The resource files generated by the bits-resolver resource plugin.
 * @type {!Array<string>}
 */
const resourceFiles = [
  ...
  describe('debug index', function() {
    ...
    it('adds script tags to the index', function() {
      for (var key in indexFiles) {
        ...
        expect(scripts.length).to.equal(expectedScripts, key + ' has incorrect number of script elements');
        ...
        expect(scripts[numResources]).to.equal('<script src=".build/gcc-defines-debug.js"></script>',
          key + ' missing gcc debug defines');

        resourceFiles.forEach(function(resource, idx, arr) {
          var scriptTag = '<script src=".build/' + resource + '"></script>';
          expect(scripts[numResources + 1 + idx]).to.equal(scriptTag,
